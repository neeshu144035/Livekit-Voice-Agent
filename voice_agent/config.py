...
SESSION_MAX_ENDPOINTING_DELAY = float(os.getenv("SESSION_MAX_ENDPOINTING_DELAY", "0.08"))
SESSION_PREEMPTIVE_GENERATION = os.getenv("SESSION_PREEMPTIVE_GENERATION", "1").strip().lower() in {"1", "true", "yes", "on"}
VAD_MIN_SPEECH_DURATION = float(os.getenv("VAD_MIN_SPEECH_DURATION", "0.015"))
VAD_MIN_SILENCE_DURATION = float(os.getenv("VAD_MIN_SILENCE_DURATION", "0.05"))
VAD_PREFIX_PADDING_DURATION = float(os.getenv("VAD_PREFIX_PADDING_DURATION", "0.12"))
ELEVENLABS_STREAMING_LATENCY = int(os.getenv("ELEVENLABS_STREAMING_LATENCY", "1"))
