...
STT_ENDPOINTING_PHONE_MS = int(os.getenv("STT_ENDPOINTING_PHONE_MS", "120"))
STT_ENDPOINTING_WEB_MS = int(os.getenv("STT_ENDPOINTING_WEB_MS", "80"))
SESSION_MIN_ENDPOINTING_DELAY = float(os.getenv("SESSION_MIN_ENDPOINTING_DELAY", "0.12"))
SESSION_MAX_ENDPOINTING_DELAY = float(os.getenv("SESSION_MAX_ENDPOINTING_DELAY", "0.65"))
SESSION_PREEMPTIVE_GENERATION = os.getenv("SESSION_PREEMPTIVE_GENERATION", "1").strip().lower() in {"1", "true", "yes", "on"}
VAD_MIN_SPEECH_DURATION = float(os.getenv("VAD_MIN_SPEECH_DURATION", "0.04"))
VAD_MIN_SILENCE_DURATION = float(os.getenv("VAD_MIN_SILENCE_DURATION", "0.30"))
VAD_PREFIX_PADDING_DURATION = float(os.getenv("VAD_PREFIX_PADDING_DURATION", "0.35"))
ELEVENLABS_STREAMING_LATENCY = int(os.getenv("ELEVENLABS_STREAMING_LATENCY", "2"))
...
async def entrypoint(ctx: JobContext):
    ...
    is_phone_call_for_llm = bool(from_number or to_number or direction == "inbound")
    if is_phone_call_for_llm and str(llm_model).strip().lower().startswith("gpt-5"):
        capped_tokens = min(max_completion_tokens, 140)
    ...
    if not is_moonshot:
        if _callable_supports_kwarg(openai.LLM, "reasoning_effort"):
            llm_kwargs["reasoning_effort"] = reasoning_effort
        if _callable_supports_kwarg(openai.LLM, "verbosity"):
    ...
    is_phone_call = bool(from_number or to_number or direction == "inbound")
    ...
    def on_conversation_item_added(event):
        nonlocal conversation_event_seen, last_activity_ts
        try:
    ...
    async def _silence_reprompt_watchdog():
        nonlocal last_activity_ts, silence_prompt_count, initial_greeting_in_progress
    ...
    try:
        ...
        if welcome_type == "agent_greets":
            ...
            try:
                if welcome_msg.strip():
                    ...
                    logger.info("Sending configured greeting via direct TTS")
                    session.say(greeting_text, add_to_chat_ctx=True, allow_interruptions=True)
                else:
                    logger.info("Generating dynamic greeting from system prompt")
                    session.generate_reply(
                        instructions=(
        ...
        for task in (transcript_monitor_task, silence_watchdog_task):
